<template>
    <div class="meeting_page">
            ...
            <el-row :gutter="30">
                <el-col :span="6">
                    <el-form-item label="公司" class="form_item">
                        <el-select collapse-tags collapse-tags-tooltip v-model="formData.companyId" multiple filterable
                ...
                <el-col :span="6">
                    <el-form-item label="服务区" class="form_item">
                        <el-select collapse-tags collapse-tags-tooltip v-model="formData.serviceAreaId" multiple
                            filterable :reserve-keyword="false" placeholder="请选择服务区" clearable>
                ...
                <el-col :span="6">
                    <el-form-item label="会议时间" class="form_item">
                        <el-date-picker value-format="YYYY-MM-DD HH:mm:ss" v-model="formData.meetingTime"
                            type="daterange" range-separator="~" start-placeholder="请选择开始日期" end-placeholder="请选择结束日期">
                        </el-date-picker>
                    </el-form-item>
                </el-col>
                <el-col :span="6">
                    <el-form-item label="会议主题" class="form_item">
                        <el-input v-model="formData.title" placeholder="请输入会议主题" clearable />
                    </el-form-item>
                </el-col>
                <el-col :span="6">
                    <el-form-item label="会议类型" class="form_item">
                        <el-select collapse-tags collapse-tags-tooltip v-model="formData.type" multiple filterable
                            :reserve-keyword="false" placeholder="请选择会议类型" clearable>
        ...
        <el-table class="custom_table" ref="tableRef" border :data="tableData" @selection-change="selectionChange"
            ...
            <el-table-column prop="serviceFullName" label="服务区" align="center" show-overflow-tooltip />
            <el-table-column prop="title" label="会议主题" align="center" show-overflow-tooltip />
            <el-table-column prop="meetTime" label="会议时间" align="center" show-overflow-tooltip />
            <el-table-column prop="master" label="会议主持人" align="center" show-overflow-tooltip />
            <el-table-column prop="member" label="参会人员" align="center" show-overflow-tooltip />
            <el-table-column prop="num" label="参会人数" align="center" show-overflow-tooltip />
            <el-table-column prop="type" label="会议类型" align="center" show-overflow-tooltip />
            <el-table-column prop="result" label="会议结论" align="center" show-overflow-tooltip />
            <el-table-column prop="cost" label="会议费用" align="center" show-overflow-tooltip />
            <el-table-column prop="recorder" label="记录人" align="center" show-overflow-tooltip />
...
import { getQueryPage, deleteMeetingManage } from "@/api/daily/meetingManage.js";
import { getCompany, getServiceArea, getServiceAreaFullName } from '@/api/informationServiceAreaApi.js';
...
const formData = reactive({
    companyId: [],
    serviceAreaId: [],
    meetingTime: [moment().format('YYYY-MM-DD') + ' 00:00:00', moment().format('YYYY-MM-DD') + ' 23:59:59'],
    title: '',
    type: []
});
/* 筛选表单数据end */
...
getServiceAreaFullName().then((res) => {
...
async function getData() {
    // const matchedIds = formData.serviceAreaId
    //     .map(name =>
    //         serviceAreaList.value.find(area => area.serviceName === name)?.serviceAreaIdList || []
    //     )
    //     .flat();
    let res = await getQueryPage({
        index: page.value,
        size: pageSize.value,
        serviceFullNameList: formData.serviceAreaId,
        companyId: formData.companyId.join(','),
        title: formData.title,
        startTime: formData.meetingTime ? formData.meetingTime[0] : null,
        endTime: formData.meetingTime ? formData.meetingTime[1] : null,
...
async function handleDelete(id) {
    ElMessageBox.confirm("是否确认删除?", "警告", {
        ...
        let res = await deleteMeetingManage({ id });
        if (res.code == 200) {
            ElMessage({
...
function doExport(imgFlag) {
    ...
    let fileName = "会议管理" + initEndTime;
    downFilePOST('/api/daily/meetingManage/export', {
        index: imgFlag ? page.value : -1,
        size: imgFlag ? pageSize.value : -1,
        serviceFullNameList: formData.serviceFullNameList,
        companyId: formData.companyId.join(','),
        title: formData.title,
        startTime: formData.meetingTime ? formData.meetingTime[0] : null,
        endTime: formData.meetingTime ? formData.meetingTime[1] : null,
