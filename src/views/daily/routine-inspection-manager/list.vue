 ...
 * @LastEditTime: 2025-02-25 13:43:28
-->
<template>
    ...
    methods: {
        ...
        handleDetail(row) {
            ...
            this.$http.callbackPost(
                ...
                { id: row.entityId, reportedByWho: 0 },
                null,
                function (result) {
                    if (result.code == 200) {
        ...
        exportData() {
            this.formInline["reportedByWho"] = 0;
            this.$http.callbackPost(
                "/daily/routineInspection/manager/export",
                this.formInline,
                { responseType: "blob" },
                function (result) {
                    ...
                    downloadElement.download = `管理员巡检记录${systemTime}.xlsx`; // 下载后文件名
        ...
        downloadTemplate() {
            this.formInline["reportedByWho"] = 0;
            this.$http.callbackPost(
                "/daily/routineInspection/manager/downloadTemplate",
                this.formInline,
                { responseType: "blob" },
                function (result) {
                    ...
                    var href = window.URL.createObjectURL(blob); // 创建下载的链接
                    downloadElement.href = href;
                    downloadElement.download = `管理员巡检记录模版${systemTime}.xlsx`; // 下载后文件名
                    document.body.appendChild(downloadElement);
                    downloadElement.click(); // 点击下载
                    document.body.removeChild(downloadElement); // 下载完成移除元素
                    window.URL.revokeObjectURL(href);
