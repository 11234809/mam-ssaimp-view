<template>
    <el-dialog class="custom_dialog" v-model="dialogVisible" :title="title" width="800" :close-on-click-modal="false"
        ...
        <el-form ref="formRef" :model="dialogForm" label-width="auto" class="custom_form" :rules="rules"
            ...
            <el-form-item label="培训主题" class="form_item" prop="title">
                <el-input :disabled="isDisabled" v-model="dialogForm.title" placeholder="请输入培训主题" clearable />
            </el-form-item>
            <el-form-item label="培训时间" class="form_item" prop="trainTime">
                <el-date-picker style="width:100%" value-format="YYYY-MM-DD" :disabled="isDisabled"
                    v-model="dialogForm.trainTime" type="date" placeholder="请选择培训时间" clearable />
            </el-form-item>
            <el-form-item label="培训老师" class="form_item" prop="teacher">
                <el-input :disabled="isDisabled" v-model="dialogForm.teacher" placeholder="请输入培训老师" clearable />
            </el-form-item>
            <el-form-item label="培训人员" class="form_item" prop="member">
                <el-input :disabled="isDisabled" v-model="dialogForm.member" placeholder="请输入培训人员" clearable />
            </el-form-item>
            <el-form-item label="人数" class="form_item" prop="num">
                <el-input-number style="width:100%;text-align:left" controls-position="right" :disabled="isDisabled"
                    v-model="dialogForm.num" :min="0" type="number" placeholder="请输入人数" clearable />
            </el-form-item>
            ...
            <el-form-item label="培训费用" class="form_item" prop="cost">
                <el-input-number style="width:100%;text-align:left" controls-position="right" :disabled="isDisabled"
                    v-model="dialogForm.cost" :min="0" type="number" placeholder="请输入培训费用" clearable />
            </el-form-item>
            <el-form-item style="float:left" label="培训内容" class="form_item" prop="content">
                <el-input :rows="3" :disabled="isDisabled" v-model="dialogForm.content" type="textarea"
                    placeholder="请输入培训内容" clearable />
            </el-form-item>
            <el-form-item label="视频上传" class="form_item">
                <el-upload :limit="10" multiple :disabled="isDisabled" v-model:file-list="dialogForm.videoFiles"
            ...
            <el-form-item style="float:left" label="附件上传" class="form_item">
                <el-upload :limit="10" multiple :disabled="isDisabled" v-model:file-list="dialogForm.attachmentFiles"
                    ...
                    :on-preview="handlePreviewAttachment" :before-upload="beforeFileAvatarUpload">
                    <el-button v-if="!isDisabled">
                        <el-icon class="el-icon--right">
                            <Plus />
                        </el-icon>点击上传
            ...
            <el-form-item label="图片上传" class="form_item">
                <el-upload :limit="10" multiple :disabled="isDisabled" v-model:file-list="dialogForm.imageFiles"
                    action="#" list-type="picture-card" accept=".jpg,.png,.jpeg" :http-request="customUploadImage"
                    :on-remove="handleRemoveImage" :on-preview="handlePreviewImage"
...
import { addTrainManage, updateTrainManage, getFileInfo } from '@/api/daily/trainManage.js';
...
const changeInput = (params) => {
    dialogForm.trainingContent = params;
}
...
const rules = reactive({
    ...
    teacher: [{ required: true, message: '请输入培训老师', trigger: 'blur' }],
    member: [{ required: true, message: '请输入培训人员', trigger: 'blur' }],
    num: [{ required: true, message: '请输入人数', trigger: 'blur' }],
    recorder: [{ required: true, message: '请输入记录人', trigger: 'blur' }],
    cost: [{ required: true, message: '请输入培训费用', trigger: 'blur' }],
            ...
            res.data.map(item => {
                dialogForm.attachmentFiles.push({
                    ...
                    url:
                        ...
                        `/pub/common/file/download?service=basTrainingFileServiceImpl&id=${item.entityId}`,
                });
            })
        })
    }
...
const saveData = async (type, formData, isContinue) => {
    await formRef.value.validate(async (valid, fields) => {
        if (valid) {
            ...
            if (type == 'add') {
                let res = await addTrainManage(formData, {
            ...
            if (type == 'edit') {
                ...
                let res = await updateTrainManage(formData, {
                    headers: { 'Content-Type': 'multipart/form-data' },
                });
                confirmLoading.value = false;
                if (res.code == 200) {
